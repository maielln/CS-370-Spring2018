...
int str2int(string s)
{
    ...
    else
    {
        ...
        {
            neg = true;
            s = s.substr(s.length(),s.length()-1);
        }
        k = 0;
        if (s.substr(0,2)=="0X")
        {
            k = hex2int(s.substr(s.length(),s.length()-2));
        }
        else if (s.substr(0,1)=="H")
        {
            k = hex2int(s.substr(s.length(),s.length()-1));
        }
        else
        {
            k = value(s);
...
void compile(int n, string filename) //may need this to compile the robots, but we may not have to if we make them their own classes
{
  //pp:parsetype;
...
void reset_hardware(int n)
{
  ...
  //with robot[n]^ do
    ...
    robot[n].ty = 0;
  do{
    robot[n].x = (rand() % 1000) - 500;
    robot[n].y = (rand() % 1000) - 500;
    dd = 1000;
